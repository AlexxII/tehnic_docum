...
$about = "Панель управления распоряжениями на проведение ВКС. При сбое, перезапустите форму, воспользовавшись соответствующей клавишей.";
  ...
  $(document).ready(function () {
    ...
    $('.del-node').click(function (event) {
      if (confirm('Вы уверены, что хотите удалить выбранный классификатор?')) {
        ...
        var node = $(".ui-draggable-handle").fancytree("getActiveNode");
        $.ajax({
          url: "/vks/control/vks-order/delete",
          type: "post",
          data: {id: node.data.id, _csrf: csrf}
        })
          .done(function () {
